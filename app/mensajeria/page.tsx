...
const DELAY_PRESETS: Record<DelayPreset, { min: number; max: number; label: string; sub: string }> = {
  lento:  { min: 45,  max: 120, label: 'Lento',  sub: '45 – 120 seg entre mensajes · Máxima seguridad' },
  normal: { min: 15,  max: 45,  label: 'Normal',  sub: '15 – 45 seg entre mensajes · Recomendado' },
  rapido: { min: 5,   max: 15,  label: 'Rápido',  sub: '5 – 15 seg entre mensajes · Mayor riesgo' },
}
...
export default function Mensajeria() {
  ...
  const previewFirstName = selectedContacts[0] ? extractFirstName(selectedContacts[0].name) : 'Juan'
  const previewText = renderTemplate(bodyText, [previewFirstName])

  // ── Step navigation ──────────────────────────────────────────────────────
  const goToPreview = () => {
    if (selected.size === 0) { toast.error('Selecciona al menos un contacto'); return }
    if (selected.size > 250) { toast.error('Máximo 250 contactos por día (límite YCloud)'); return }
    fetchTemplates()
    setStep('preview')
  }
  ...
  const startSend = async () => {
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      // Gaussian delay before next message
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-6 sm:py-8 dark-mode-transition">
        <div className="max-w-5xl mx-auto px-3 sm:px-4">
          ...
          <div className="mb-6">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm">Envíos masivos de WhatsApp con sistema anti-spam inteligente</p>
          </div>
              ...
              <div key={s.key} className="flex items-center flex-1">
                <div className="flex flex-col items-center">
                  ...
                  <span className={`text-xs font-semibold mt-1 hidden sm:block ${
                    i === stepIdx ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400 dark:text-gray-500'
                  }`}>{s.label}</span>
                </div>
                {i < STEPS.length - 1 && (
            ...
            <div className="space-y-4">
              <Card variant="elevated" className="p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      placeholder="Buscar nombre, empresa, tel..."
                      value={search}
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  ...
                  {(search || filterSegment || filterStatus) && (
                    <button onClick={() => { setSearch(''); setFilterSegment(''); setFilterStatus('') }} className="text-xs text-gray-400 hover:text-red-500 underline transition">
                      Limpiar
                    </button>
                  )}
                </div>
              ...
              {selected.size > 250 && (
                <div className="flex items-center gap-2 p-3 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-700 rounded-xl text-sm text-amber-800 dark:text-amber-300">
                  ...
                  Límite diario de YCloud: 250 conversaciones. Tienes {selected.size} seleccionados — reduce la selección.
                </div>
              )}

              <div className="flex justify-end">
                <Button variant="primary" onClick={goToPreview} disabled={selected.size === 0 || selected.size > 250}>
              ...
              {selectedTemplate && (
                <Card variant="elevated" className="p-5">
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-3">
                    ✦ <code>{'{{1}}'}</code> → primer nombre del contacto — cada mensaje será único
                  </p>
                </Card>
              )}
          ...
          {step === 'config' && (
            <div className="space-y-4">
              ...
              <Card variant="elevated" className="p-5">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 mb-4">Distribución gaussiana — los delays varían de forma natural como un humano</p>
                <div className="grid grid-cols-3 gap-3">
              ...
              <div className="p-4 bg-gray-100 dark:bg-gray-800 rounded-xl flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-gray-700 dark:text-gray-200">
                    Tiempo estimado: <span className="text-primary-600 dark:text-primary-400">{estimatedTime(selectedContacts.filter(c => c.phone).length, delayPreset)}</span>
                  </p>
                  <p className="text-xs text-gray-400 dark:text-gray-500">
                    {selectedContacts.filter(c => c.phone).length} mensajes · plantilla: {selectedTemplate?.name}
                  </p>
                </div>
              </div>
              ...
              <Card variant="elevated" className="p-5">
                <div className="flex items-center justify-between mb-3">
                  <h2 className="font-bold text-gray-900 dark:text-white">
                    {isDone ? 'Envío completado' : isSending ? 'Enviando...' : 'Pausado'}
                  </h2>
                ...
                {countdown !== null && !isPaused.current && (
                  <div className="flex items-center gap-2 p-2 bg-blue-50 dark:bg-blue-950/30 rounded-lg mb-3 text-sm text-blue-700 dark:text-blue-300">
                    <Zap size={14} />
                    Próximo mensaje en <strong>{countdown}s</strong>
                    <span className="text-xs text-blue-400 ml-1">(delay anti-spam)</span>
                  </div>
                )}
                  ...
                  {log.map(entry => (
                    <div key={entry.contactId} className={`flex items-center gap-3 px-5 py-3 ${
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        {entry.status === 'failed' && <span className="text-xs text-red-600 dark:text-red-400 truncate max-w-[120px] block">{entry.error}</span>}
                        {entry.status === 'skipped' && <span className="text-xs text-gray-400 truncate max-w-[120px] block">{entry.error}</span>}
